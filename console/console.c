...
int attrib = (FB_BLACK << 4) | FB_WHITE;
int csr_x = 0, csr_y = 0;
...
void clear_screen(void)
{
  ...
  for(i = 0; i < CONSOLE_HEIGHT; i++)
    memsetw ( framebuffer + i * CONSOLE_WIDTH, blank, CONSOLE_WIDTH);
...
void scroll(void)
{
  ...
  if(csr_y >= CONSOLE_HEIGHT) {
      ...
      memcpy ((byte*)framebuffer
              , (byte*)framebuffer + temp * CONSOLE_WIDTH
              , (CONSOLE_HEIGHT - temp) * CONSOLE_WIDTH * 2);

      /* Finally, we set the chunk of memory that occupies
        *  the last line of text to our 'blank' character */
      memsetw (framebuffer + (CONSOLE_HEIGHT - temp) * CONSOLE_WIDTH
              , blank
              , CONSOLE_WIDTH);
...
void move_csr(void)
{
  ...
  outb(FB_DATA_PORT, temp >> 8);
  outb(FB_COMMAND_PORT, FB_LOW_BYTE_COMMAND);
  outb(FB_DATA_PORT, temp);
}
...
void put_char(unsigned char c)
{
  ...
  /* Any character greater than and including a space, is a
    ...
    *  Index = [(y * width) + x] */
      ...
      where = framebuffer + (csr_y * CONSOLE_WIDTH + csr_x);
